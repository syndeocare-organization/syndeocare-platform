...
export function ApplyButton({ locale, shiftId, existingStatus }: { locale: Locale; shiftId: string; existingStatus: string | null }) {
  ...
  if (status) return <span className={`inline-flex min-h-10 items-center gap-2 rounded-full px-4 text-xs font-black ${applicationStatusTone(status)}`}><CheckCircle2 className="size-4" />{applicationStatusLabel(status, locale)}</span>;
