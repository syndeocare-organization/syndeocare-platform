    ...
    <footer className="border-t border-brand-950/8 bg-white">
      <div className="page-shell grid gap-8 py-12 md:grid-cols-[1fr_auto] md:items-end">
        ...
        <nav className="flex flex-wrap gap-x-6 gap-y-3 text-sm font-semibold text-slate-700" aria-label="Footer">
          <a className="hover:text-brand-700" href={localePath(locale, "/privacy")}>{t.footer.privacy}</a>
          <a className="hover:text-brand-700" href={localePath(locale, "/support")}>{t.nav.support}</a>
          <a className="hover:text-brand-700" href={localePath(locale, "/delete-account")}>{t.footer.delete}</a>
        </nav>
      </div>
