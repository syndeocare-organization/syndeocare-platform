...
export function DocumentCenter({ locale, documents }: { locale: Locale; documents: VerificationDocument[] }) {
  ...
  return (
    <div className="grid gap-6 lg:grid-cols-[.75fr_1.25fr]">
      <form onSubmit={upload} className="app-border app-surface h-fit rounded-[1.75rem] border p-6 shadow-sm sm:p-7">
        ...
        <p className="mt-2 text-sm leading-7 app-text-muted">{isArabic ? "ملفاتك خاصة ولا يراها إلا أنت وفريق التحقق." : "Your files are private and visible only to you and the verification team."}</p>
        <div className="mt-6 grid gap-5">
          ...
          <div><Label htmlFor="documentFile">{isArabic ? "الملف" : "File"}</Label><Input id="documentFile" name="file" type="file" accept="application/pdf,image/jpeg,image/png" required className="py-2 file:me-3 file:rounded-full file:border-0 file:bg-brand-50 file:px-3 file:py-2 file:text-xs file:font-black file:text-brand-700" /><FieldHint>PDF, JPG, PNG · {isArabic ? "حتى 10 ميجابايت" : "up to 10 MB"}</FieldHint></div>
      ...
      <section className="grid content-start gap-4" aria-label={isArabic ? "المستندات المرفوعة" : "Uploaded documents"}>
        {documents.length ? documents.map((document) => (
          <article key={document.id} className="app-border app-surface rounded-[1.5rem] border p-5 shadow-sm sm:p-6">
            <div className="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
              <div className="flex min-w-0 items-start gap-4"><span className="app-surface-muted grid size-11 shrink-0 place-items-center rounded-2xl app-text-muted"><FileText className="size-5" /></span><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><h3 className="font-black">{labels(document.type, locale)}</h3><span className={`rounded-full px-2.5 py-1 text-[10px] font-black ${statusTone(document.status)}`}>{labels(document.status, locale)}</span></div><p className="mt-1 truncate text-sm app-text-muted" dir="auto">{document.originalFilename}</p><p className="bidi-isolate mt-2 text-xs app-text-muted">{new Intl.DateTimeFormat(locale === "ar" ? "ar-SA" : "en-US", { dateStyle: "medium" }).format(new Date(document.createdAt))}{document.expiresOn ? ` · ${isArabic ? "ينتهي" : "expires"} ${document.expiresOn}` : ""}</p></div></div>
        ...
        )) : <div className="app-border app-surface rounded-[1.75rem] border border-dashed px-6 py-16 text-center"><FileText className="mx-auto size-10 app-text-muted" /><h2 className="mt-5 font-black">{isArabic ? "لم ترفع مستندات بعد" : "No documents uploaded"}</h2><p className="mt-2 text-sm app-text-muted">{isArabic ? "ابدأ بالهوية والترخيص المهني لتسريع التحقق." : "Start with your ID and license to speed up verification."}</p></div>}
      </section>
    </div>
  );
}
