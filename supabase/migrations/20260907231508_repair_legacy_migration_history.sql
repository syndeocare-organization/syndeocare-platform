...
values ('202608240001','initial_platform',array[$scbaseline0$-- SyndeoCare platform foundation.
...
insert into supabase_migrations.schema_migrations(version,name,statements)
values ('202608250001','fix_profile_phone_constraint',array[$scbaseline1$-- Align the database phone constraint with the shared onboarding contract.
...
on conflict (version) do update set name=excluded.name, statements=excluded.statements;
insert into supabase_migrations.schema_migrations(version,name,statements)
values ('202608250002','fix_applications_policy_recursion',array[$scbaseline2$create or replace function public.is_professional()
returns boolean
language sql
stable
security definer
...
on conflict (version) do update set name=excluded.name, statements=excluded.statements;
insert into supabase_migrations.schema_migrations(version,name,statements)
values ('202608300001','harden_application_submission',array[$scbaseline3$-- A professional may only apply to a currently published future shift.
